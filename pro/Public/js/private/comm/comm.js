...
//
//      }
        $('.tiRight2>svg').on('click',function(){
            if($('.hideUlis').height()==0){
                // $('.hideUlis').css('height','280px')
                $('.hideUlis').css('height',($('.hideUlis').find('li').length*40)+'px')
            }else{
                $('.hideUlis').css('height','0px')
            }
        })
    }
    ...
    obj.off = function () {
        if(localStorage.getItem('ofs')){
            if(localStorage.getItem('ofs')==1){
                $('body').addClass('bgcor');
                $('.off img').attr('src','/public/img/of2.png')
            }
        }
        $('.off').on('click',function(){
            if(localStorage.getItem('ofs')){
                if(localStorage.getItem('ofs')==0){
                    localStorage.setItem('ofs',1);
                    $('body').addClass('bgcor');
                    $('.off img').attr('src','/public/img/of2.png')
                }else{
                    localStorage.setItem('ofs',0);
                    $('body').removeClass('bgcor');
                    $('.off img').attr('src','/public/img/of1.png')
                }
            }else{
                localStorage.setItem('ofs',1);
                    $('body').addClass('bgcor');
    ...
    obj.bottom = function () { //判断底部到底了没
        if($(window).height()>($('.footer').offset().top+300)){
    ...
    obj.getCount = function(){
        $.ajax({
            url:'/Home/index/getOnline',
            type:'get',
            success:function(req){
                $('.userCount').html(req.online)
            }
        })
    }
        ...
        table.dataTable({
            "ajax": {
                ...
                "dataSrc": function ( json ) {
                    var data = json.data.page?json.data.page.data:json.data.funds.data?json.data.funds.data:json.data.funds?json.data.funds:'null';
                    var data = data?data:{};
    ...
    obj.formTime = function (data) {
        ...
        return m+'-'+obj.timer(y)+'-'+obj.timer(d)+' '+obj.timer(h)+':'+obj.timer(f)+':'+obj.timer(s);
    }
    ...
    obj.langChange = function(){
        $('.langs').on('click',function(){
            $.ajax({
                url:'/Home/index/langChange',
                type:'get',
                dataType:'json',
                data:{
                    l:$(this).attr('data-lang')
                },
                success:function(req){
                   if(req.status==1){
                    alert(req.lang+req.info)
                    setTimeout(function(){
    ...
    obj.alert=function () {
        window.alert=function (val) {
            ...
            if(document.getElementById('alert1')==null ||document.getElementById('alert1')=='undefined'){
                ...
                div.setAttribute('style',"height:40px;background:rgba(0,0,0,.5);position:fixed;top:0px;left:0;color:#fff;z-index:999;padding:10px;opacity:1;border-radius:5px;    box-sizing: border-box;");
